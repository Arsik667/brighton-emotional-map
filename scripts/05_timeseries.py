...
def plot_seasonality(profile: pd.DataFrame, path, lang: str = "en") -> None:
    ...
    ax.set_ylabel(L["net_sentiment"])
    ax.set_title(L["seasonality_title"], fontsize=13, pad=14)
...
def plot_monthly_trend(monthly: pd.DataFrame, path, lang: str = "en") -> None:
    ...
    for ax, district in zip(axes, districts):
        ...
        if len(weak):
            ax.scatter(weak["month"].dt.to_timestamp(), weak["net_sentiment"],
                       ...
                       label=L["low_n"].format(threshold=MIN_OBSERVATIONS))
    ...
    axes[0].set_title(L["trend_title"], fontsize=12, pad=14)
    fig.tight_layout()
    fig.savefig(path, dpi=140)
    plt.close(fig)
...
def main() -> None:
    ...
    parser.add_argument("--lang", default="en", choices=["en", "ru"],
                        help="язык подписей на графиках (README на GitHub английский)")
    ...
    console_months = get_months("ru")  # в консоль пишем по-русски
    ...
    for m, row in by_month.iterrows():
        ...
        if m == 8:
            ...
            mark = "  <- Fringe"
        print(f"  {console_months[m-1]}  {row['value']:+.3f}  n={int(row['n']):>4}  "
              f"{'█'*filled}{mark}")
    ...
    plot_seasonality(profile, FIGURES_DIR / "seasonality.png", lang=args.lang)
    plot_monthly_trend(monthly, FIGURES_DIR / "monthly_trend.png", lang=args.lang)
